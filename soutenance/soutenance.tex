...
\begin{block}{Les clefs d'internet}
	\begin{itemize}
		\item SSL/TLS (port 443);
		\item évaluation du niveau de sécurité d'internet;
...
\frame	{
    \frametitle{Récupération des certificats II}
    	\begin{block}{Algorithme}
			...
			\begin{algorithm}[H]
				\label{alg:recupCertif}
				 ...
				 \Sortie{Certificats et clefs publiques RSA des adresses}
				 ...
				 \PourTous{adresses de f}{
					...
					\Si{!echec de capture de session}{
						on extrait le certificat et la clef publique RSA\;
					}	 
					}
				}
				\Retour{certificats et clefs publiques RSA}
...
\frametitle{Certificats récupérés}
	\begin{block}{Failles}
		\begin{itemize}
			\item pas de Common Name ;
			\item que le Serial Number.\\
		\end{itemize}
	\end{block}
}

\frame{
    \frametitle{Gestion des doublons I}
	\begin{block}{Gestion des doublons}
	\begin{itemize}
		\item script perl ;
		\item si une empreinte déjà présente : on la stocke dans un autre dossier ;
...
\frame{
    \frametitle{Gestion des doublons II}	
...
\begin{block}{Base de données}
   	\begin{itemize}
		\item récupération des facteurs communs - clefs publiques;
...
\begin{block}{Statistiques}
   	\begin{itemize}
		...
		\begin{itemize}
			\item taille de clefs;
			\item émetteurs, sujets : CISCO - 6280 (R) - 40 (V).\\
		\end{itemize}
	\end{itemize}
\end{block}
}
...
\frametitle{OpenSSL}
	\begin{block}{Observation}
		...
		\begin{itemize}
			\item documentation;
			\item code développé en mode réactif;
			\item lisibilité du code;
			\begin{itemize}
			\item exemple de code
			\begin{verbatim}
				n=((p[0]&0x7f)<<8)|p[1];				
			\end{verbatim}
			\item nom des fichiers : 
			\verb+s3_+ ;
			\end{itemize}
...
\item \textbf{fin de validité de certificats} : 2020 -- 2030.
\end{itemize}
\end{block}
}
...
\frame{
\frametitle{Chiffrement et protocoles I}
	\begin{block}{RSA-OAEP}
		laïus RSA-OAEP (fonctionnement, +, -)
	\end{block}

\framebreak

	\begin{block}{Manger's attack}
		\begin{itemize}
			\item OpenSSL 1.0.0 ;
			\item OAEP : défaillant ?
			\item contrôler la taille des paramètres à hacher ;
			\item sur serveur : variations de délais ;
			\item systèmes embarqués : plus problématique. \\
		\end{itemize}
	\end{block}
}

\frame{
\frametitle{Chiffrement et protocoles II}
\framesubtitle{CBC}

	\begin{figure}[H]
		\centering
		\includegraphics[width=7cm]{CBC_chiff.png}
		\label{CBC_chiff}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=7cm]{CBC_dechiff.png}
		\label{CBC_dechiff}
	\end{figure}
}
\frame{
\frametitle{Chiffrement et protocoles III}

	\begin{block}{\textit{Man in the Middle}}
		\begin{itemize}
			\item attaque à clair choisi ;
			\item récupérer cookies de session.\\
		\end{itemize}
	\end{block}
	\begin{block}{Problème}
		\begin{itemize}
			\item SSL/TLS chiffre un canal de communication ;
			\item problème d'IV.\\
		\end{itemize}
	\end{block}
	...
	\begin{block}{Recommandations}
		\begin{itemize}
			\item ne pas utiliser CBC ;
			\item concaténer tous les objets.\\
		\end{itemize}
	\end{block}
...
\frame{
\frametitle{Signature et authentification I}
\begin{block}{Définition}
\begin{itemize}
\item Juridiquement, une signature électronique a même valeur qu'une signature manuscrite.
...
\frame{
\frametitle{Signature et authentification II}
\begin{block}{Audit : Attaque par injection de fautes sur les certificats RSA.}
\begin{itemize}
\item \textbf{Description} : L'attaque se fait sur des morceaux de la signature afin de récupérer la clef privée bit à bit.
\item \textbf{Comment?} : Du bon matériel, surtout au niveau de la mémoire vive (i.e. Système Linux avec une architecture SPARC) et un oracle (e.g. système de prédictions) permettant de fabriquer la clef.
\item \textbf{Temps de l'attaque} : une centaine d'heures.
\end{itemize}
\end{block}
}

\frame{
\frametitle{Signature et authentification III}
\begin{block}{Audit : Attaque par injection de fautes sur les certificats RSA.} 
\begin{itemize}
\item \textbf{un problème dans le code OpenSSL?} : la fonction \texttt{Fixed\_Window\_Exponentiation} utilise des milliers de multiplications, qui est l'opération la plus sensible en cas de dégradation du micro-processeur.
...
\item \textbf{conséquences :} à moins que l'attaquant n'ait accès physiquement à votre machine les risques sont faibles. Cependant l'Université du Michigan cherche un moyen de faire des injections à distance à base d'\texttt{impulsions lasers}.
\end{itemize}
\end{block}
}
...
\subsection{Qualys}
\frame{
\frametitle{Qualys SSL Labs}
\begin{block}{Description}
\begin{itemize}
\item Qualys est une plateforme multi-sécurité :
\item Le projet SSL Labs permet de :
\begin{itemize}
\item tester l'implémentation SSL du navigateur;
\item tester la configuration et les certificats d'un serveur;
\item lister les bonnes pratiques sur les déploiements SSL/TLS.
\end{itemize}
\end{itemize}
\end{block}
...
\frame{
\frametitle{Qualys SSL Labs}
\begin{block}{Analyse des serveurs}
L'analyse porte sur les points suivants :
\begin{itemize}
\item contrôle du certificat;
\item protocoles supportés;
\item échange des clés;
\item qualité de chiffrement; 
...
\begin{block}{Tests sur un certificat vulnérable}
SSL Report : \texttt{spXXXX.XXXX.net} (\texttt{85.XXX.XXX.33})
\begin{itemize}
\item Notation : F
\item Certificat non sûr $\rightarrow$ Non-confiance dans la chaîne de certification
\item Sensible à une attaque de type CRIME
\item Protocoles obsolètes $\rightarrow$ TLS\_RSA\_EXPORT\_WITH\_RC4\_40\_MD5
\item Taille de clé insuffisante $\rightarrow$ RSA 1024 bits
\item Re-négociation sécurisée non supportée.
\item Compression TLS non-securisée
\item Généré en Janvier 2014 - Expire en Janvier 2038
\end{itemize} 
\end{block}
...
\begin{block}{Bilan du projet}
	\begin{itemize}
		...
		\item évaluation du code OpenSSL.\\
	\end{itemize}
\end{block}
